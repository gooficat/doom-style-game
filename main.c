...
static const unsigned short scrW = 640;
static const unsigned short scrH = 320;


void pixel(int x, int y, uint32_t color) {
	if (x < 0 || x >= scrW || y < 0 || y >= scrH) return;
...
void drawWall(int x1, int x2, int b1, int b2, int t1, int t2, uint32_t color) {
    int x, y;
...
void drawSectors() {
    ...
    for (s = 0; s < sectCount; s++) {
        // printf("sector: %d %d %d %d\n", sectors[s].idx, sectors[s].end, sectors[s].z1, sectors[s].z2);
        for (w = sectors[s].idx; w < sectors[s].end; w++) {
            ...
            drawWall(wx[0], wx[1], wb[0], wb[1], wt[0], wt[1], walls[w].c);
        }
    }
}
...
void main() {
	...
	window = SDL_CreateWindow("doom-style game!", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, scrW, scrH, SDL_WINDOW_SHOWN);
